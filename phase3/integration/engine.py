...
Integrates:
  ...
  - Phase 3: OTA configuration updates (ota_handler.py)
"""
...
try:
    ...
    from mock_mqtt_manager import MockMQTTManager as MQTTManager
...
async def run_engine() -> None:
    ...
    # ── OTA topic subscriptions ───────────────────────────────────────────────
    def _make_ota_callback(room: Room) -> Any:
        async def _ota_cb(topic: str, raw: bytes) -> None:
            result = room._ota.verify_and_apply(
                raw, room.alpha, room.beta, room.current_version
            )
            if result is None:
                return
            if "security_alert" in result:
                await mqtt_manager.publish(
                    "v1/devices/me/telemetry", result, qos=cfg.mqtt.qos
                )
                logger.warning("[%s] Security alert published to ThingsBoard", room.id)
            else:
                room.alpha           = result["new_alpha"]
                room.beta            = result["new_beta"]
                room._ota_version    = result["new_version"]
                room.current_version = result["new_version"]
                await mqtt_manager.publish(
                    "v1/devices/me/attributes",
                    room._ota.build_version_attribute(room.current_version),
                    qos=cfg.mqtt.qos,
                )
                logger.info(
                    "[%s] OTA applied — version=%s alpha=%.4f beta=%.4f",
                    room.id, room.current_version, room.alpha, room.beta,
                )
        return _ota_cb

    for room in rooms.values():
        for topic in room._ota.get_ota_topics():
            mqtt_manager.subscribe(topic, callback=_make_ota_callback(room))
    logger.info("OTA subscriptions registered for %d rooms", len(rooms))
